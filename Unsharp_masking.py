...
if __name__ == '__main__':
    ...
    for path in image_files:
        with Image.open(path) as img:
            ...
            # Save images
            Image.fromarray(sharpened_1.astype(np.uint8)).save(
                f"./img/{name_only}_scale1{ext}")
            Image.fromarray(sharpened_2.astype(np.uint8)).save(
                f"./img/{name_only}_scale2{ext}")
